...
namespace App\DependencyInjection\Override;
...
final class FieldsBuilder implements FieldsBuilderInterface
{
    ...
    private function getResourceFieldConfiguration(?string $property, ?string $fieldDescription, ?string $deprecationReason, Type $type, string $rootResource, bool $input, Operation $rootOperation, int $depth = 0, bool $forceNullable = false): ?array
    {
        try {
            ...
            // TODO: This override is usefull to fetch the correct Operation by its name
            $operationName = GraphQLExtension::getOperationName($property, $type, $rootResource, $resourceClass, $input, $rootOperation, $isCollectionType);
            ...
            if ($resourceClass && $this->resourceClassResolver->isResourceClass($resourceClass) && $rootOperation->getClass() !== $resourceClass) {
                ...
                $resolverOperation = $resourceMetadataCollection->getOperation($operationName, $isCollectionType);
            ...
            if (!$input && !$rootOperation instanceof Mutation && !$rootOperation instanceof Subscription && !$isStandardGraphqlType && $isCollectionType) {
                ...
                if (!empty($resourceClass)) {
                    $resourceMetadataCollection = $this->resourceMetadataCollectionFactory->create($resourceClass);
                    try {
                        $operation = $resourceMetadataCollection->getOperation($operationName, true);
                    } catch (OperationNotFoundException $e) {
                    }
                }

                // TODO: This allows to specify for any subresource if we want pagination enabled or not
                if ($operationName !== null && $operation instanceof Operation) {
                    if ($this->pagination->isGraphQlEnabled($operation)) {
                        $args = $this->getGraphQlPaginationArgs($operation);
                    }
                } else {
            ...
            if ($isStandardGraphqlType || $input) {
                ...
                // TODO: operation should be used instead of resolverOperations in case we have stated already this was a collection query and initialized the resolver previously
                $resolve = ($this->collectionResolverFactory)($resourceClass, $rootResource, $operation ?? $resolverOperation);
            } else {
                $resolve = ($this->itemResolverFactory)($resourceClass, $rootResource, $resolverOperation);
            }

            // TODO: this override is usefull to provide additionnal args on any property inside the schema
            if ($operationName !== null) {
                $_operation = ($operation ?? $resolverOperation ?? null);
                $operationArgs = $_operation?->getArgs();
                $args = $this->resolveResourceArgs($operationArgs ?? [], $_operation) + $args;
            }
